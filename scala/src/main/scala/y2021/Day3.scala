...
/**
 * https://adventofcode.com/2021/day/3 TODO: Better binary representation This
 * implementation is confusing AF
 */
object Day3 {
  ...
  case class BinaryReport(bits: Seq[Binary]) {
    ...
    def merge(other: BinaryReport): BinaryReport = {
      ...
      BinaryReport(this.bits.zip(other.bits).map { case (a, b) =>
        Binary(a.value + b.value, a.zeroes + b.zeroes, a.ones + b.ones)
      })
    }

    def gamma: Int = bits
      .map(_.mostCommon)
      .foldLeft((0d, length - 1)) { (acc, d) =>
        (acc._1 + (d * math.pow(2, acc._2)), acc._2 - 1)
      }
      ._1
      .toInt

    def epsilon: Int = bits
      .map(_.leastCommon)
      .foldLeft((0d, length - 1)) { (acc, d) =>
        (acc._1 + (d * math.pow(2, acc._2)), acc._2 - 1)
      }
