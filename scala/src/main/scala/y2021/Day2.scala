...
object Day2 {
  ...
  def part2(commands: Seq[(String, Int)]): Int = {
    ...
    val finalPost = commands.foldLeft[(Int, Int, Int)](0, 0, 0) {
      (positionWithAim, command) => {
        val r = command match {
          case ("forward", x) => (positionWithAim._1 + x, positionWithAim._2 + calculateDepthChange(x, positionWithAim._3), positionWithAim._3)
